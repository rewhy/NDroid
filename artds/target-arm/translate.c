...
static inline void gen_bx_im(DisasContext *s, uint32_t addr)
{
    ...
    /** START DECAF ADDITIONS **/
    //SEE ALSO: gen_bx, gen_goto_tb, and gen_jmp
    //gen_bx_im is called when there is a branch to a known target
    // however, it is also called when single_stepping is enabled
    // in gen_jmp(). 
    //NOTE: We insert the call here - before the PC has been updated
    
    //First we handle the insn end callback
...
static inline void gen_bx(DisasContext *s, TCGv var)
{
    ...
    /** START DECAF ADDITIONS **/
    //SEE ALSO: gen_bx_im, gen_goto_tb, and gen_jmp
    //gen_bx is called when there is an indirect branch 
    //NOTE: We insert the callbacks here - after the PC has been updated
    // to maintain compatibility with DECAF parent
    // it is before the thumb state has been updated though
    // so we can reuse var - hope this is not too confusing for the user?
    //NOTE: This also handles the store_reg functions
    ...
    next_pc = -1; //we set next_pc to -1 meaning it is an indirect jump
    if (DECAF_is_BlockEndCallback_needed(cur_pc, next_pc))
    {
      TCGv_ptr tmpTb = tcg_const_ptr((tcg_target_ulong)s->tb);
      TCGv tmpFrom = tcg_temp_new();
      //TCGv tmpTo = tcg_temp_new(); // we reuse var

      tcg_gen_movi_tl(tmpFrom, cur_pc);
      //tcg_gen_movi_tl(tmpTo, next_pc);

      gen_helper_DECAF_invoke_block_end_callback(cpu_env, tmpTb, tmpFrom, var);
...
{
    /** START DECAF ADDITIONS **/
    //NOTE: We do not handle this call even though this is 
    // technically a jump. The reasoning is because this
    // function seems to be only used for interrupt or 
    // exception handling (or in get_goto_tb, which is handled
    // separately). Exceptions and interrupts are asynchronous 
    // and are not considered end of basic blocks
    /** END DECAF ADDITIONS **/ 
...
static inline void gen_goto_tb(DisasContext *s, int n, uint32_t dest)
{
    ...
    /** START DECAF ADDITIONS **/
    //SEE ALSO: gen_bx, gen_bx_im, and gen_jmp
    //gen_goto_tb is called when there is an direct branch 
    // so multiple TBs can be linked together for performance 
    // notice how it seems that blocks are chained only when they
    // are in the same code page
    //NOTE: We insert the callback before the tcg_gen_goto_tb call
    // or the set_pc call otherwise we might not even get the callback
...
{
    /** START DECAF ADDITIONS **/
    //SEE ALSO: gen_bx, gen_bx_im, gen_goto_tb
    //NOTE: We don't actually do anything here since gen_jmp
    // uses gen_bx_im and gen_goto_tb both of which are handled
    // separately 
    /** END DECAF ADDITIONS **/ 
...
static void disas_arm_insn(CPUState * env, DisasContext *s)
{
    ...
    /** START DECAF ADDITIONS **/
    //The instruction begin is a bit tricky since 
    // the current instruction could be an illegal instruction 
    //We have to leave it up to the user to deal with 
    // these special cases
    //TODO: Fiddle with the TCG to disable the gen_helper call
    // if the instruction turns out to be an illegal_op
    //SEE: insn_end callback in the illegal_op section below
    cur_pc = s->pc;
    next_pc = (-1);
    ...
    if ((insn & 0x0f900000) == 0x03000000) {
        ...
        case 0xf:
            /* swi */
            /** START DECAF ADDITIONS **/
            if (insn == 0xef000000) // this is swi or svc #0
            {
        ...
        illegal_op:
            /** START DECAF ADDITIONS **/
            //Until we get the TCG logic to disable the instruction begin
            // we need to signal that the instruction was in error
            //We do that here by issuing an instruction BEGIN with -1 as the cur_pc
            if (DECAF_is_callback_needed(DECAF_INSN_BEGIN_CB, cur_pc, next_pc))
            {
              TCGv tmpPC = tcg_temp_new();
    ...
    /** START DECAF ADDITIONS **/
    //If we are here, then the current instruction was successfully disassembled
    // so we can just add in the callback code
    //NOTE: It is possible that a JMP was generated prior to this which means
    // that the callback logic here will never be invoked, however we do ensure
    // that an instruction end is called when block_end is called. This should
    // take care of the jumps
...
{
    /** START DECAF ADDITIONS **/
    //We do not do any insn_begin, or end callbacks here
    // because this is called from disas_thumb
    /** END DECAF ADDITIONS **/
...
static void disas_thumb_insn(CPUState *env, DisasContext *s)
{
    ...
    /** START DECAF ADDITIONS **/
    //SEE ALSO: disas_arm_insn
    //TODO: Fiddle with the TCG to disable the gen_helper call
    // if the instruction turns out to be an illegal_op
    //SEE: insn_end callback in the illegal_op section below
    cur_pc = s->pc | 1; //set the low bit of the PC to signify thumb mode
    next_pc = (-1);
    if (DECAF_is_callback_needed(DECAF_INSN_BEGIN_CB, cur_pc, next_pc))
    {
      TCGv tmpPC = tcg_temp_new();
