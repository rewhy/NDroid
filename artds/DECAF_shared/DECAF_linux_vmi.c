...
gva_t DECAF_get_current_task_struct(CPUState* env)
{
  gva_t threadinfo = 0;
  gva_t curtask = 0;
	// DECAF_printf("DBG: DECAF_get_current_task_struct() env = 0x%08x\n", (int)env);
  if (env == NULL)
  {
    return (0);
  }
  ...
  threadinfo = DECAF_getESP(env) & ~8191;
	// DECAF_printf("DBG: DECAF_get_current_task_struct() threadinfo = 0x%08x, thread_info_task_offset = 0x%x\n", \
			threadinfo, thread_info_task_offset);
  DECAF_read_mem(env, threadinfo + thread_info_task_offset, &curtask, 4);
	// DECAF_printf("DBG: DECAF_get_current_task_struct() curtask = 0x%08x\n", curtask);
  return (curtask);
}
