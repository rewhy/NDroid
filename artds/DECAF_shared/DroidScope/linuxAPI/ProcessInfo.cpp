...
#include "DECAF_shared/DroidScope/taintTracker/register.h"
...
int getModuleStartEndAddress(gpid_t pid,gva_t* pStartAddr, gva_t* pEndAddr, const char* moduleName){
    return processInfoMap.getModuleStartEndAddress(pid, pStartAddr, pEndAddr, moduleName);
}


int ProcessInfoMap::getModuleStartEndAddress(gpid_t pid, gva_t* pStartAddr, gva_t* pEndAddr, const char* moduleName)
{
    ProcessInfo* pInfo = findProcessByPID(pid);
    if (pInfo==NULL) {
        return (NULL_POINTER_ERROR);
    ...
    gva_t start=0, end=0;
    //whether boot.oat founded
    int flag = 0;
    ModuleNode* i = pInfo->modules;
    for(; i != NULL; i = i->next){
        ModuleInfo* pModInfo = (ModuleInfo*)(i->moduleInfo);
        if(strcmp(pModInfo->getName().c_str(), moduleName)==0)
        {
            flag = 1;
            //firt boot.oat module
            if (start == 0){
                start = i->startAddr;
            }
            end = i->endAddr;
        }else{
            //other module, and oat has been found before, break
            if (flag){
                break;
            }
        }
    }
    //only when found, update the input parameters
    if (flag){
        *pStartAddr=start;
    	*pEndAddr=end;
    }
...
extern gpid_t pid_wanted;

bool isThread(gva_t tgid,gva_t tid){
    if (tgid!=pid_wanted) {
        return 0;
    }else{
        return (processInfoMap.isThread(tid));
    }
}


bool ProcessInfoMap::isThread(gva_t tid)
{
     _ProcessInfoMap::const_iterator it;
    it=processInfoMap.find(pid_wanted);
    if (it == processInfoMap.end())
    {
        return 0;
    }else{
...
void ProcessInfoMap::updateTaintRegs(gpid_t pid){
    _ProcessInfoMap::iterator it;
    it = processInfoMap.find(pid);
    if (it == processInfoMap.end())
    {
    ...
    DECAF_printf("DO_FORK!!!!!!!!!!!!!!!\n"); // test -- zhouhao

    ProcessInfo* pTemp = it->second;
    ThreadNode* pt = NULL;
    for (pt = pTemp->threads; pt != NULL; pt = pt->next)
    {
        if(find_map(pt->tid)==NULL)
        {
            DECAF_printf("Thread %d is created!!!\n",pt->tid);
            insert_map(pt->tid);
        }
    }
}

void updateTaintRegs(gpid_t pid){
  return processInfoMap.updateTaintRegs(pid);
}

bool ProcessInfoMap::isBpHit(gpid_t pid,gva_t addr){
    _ProcessInfoMap::iterator it;
    it = processInfoMap.find(pid);
    if (it == processInfoMap.end())
    {
    ...
    ThreadNode* pt = NULL;
    for (pt = pTemp->threads; pt != NULL; pt = pt->next){
        struct tls_taint* tmp=find_map(pt->tid);
        if (tmp) {
            if (tmp->breakpoint==addr) {
                return true;
            }
        }
    }
...
bool isBpHit(gpid_t pid,gva_t addr)
{
    return processInfoMap.isBpHit(pid,addr);
}
#endif
/*ARTDS END  */
