...
#include "Context.h"
/* ARTDS START */
#include "DECAF_shared/DroidScope/taintTracker/trace.h"
...
/* ARTDS START*/
target_ulong curProcessUID = (-1);
extern target_ulong uid_wanted;
/* ARTDS END*/
...
inline target_ulong getCurrentUID()
{
  return (curProcessUID);
}
...
/* ARTDS START */
void updateThreadByPID(CPUState* env, gpid_t pid)
{
    gpid_t t_pid;
    gpid_t t_tgid;
    gva_t i = DECAF_get_current_process(env);
    gva_t j = i;
    clearThreads(pid);
    do
    {
...
void artdsUpdateTracedProcessByTask(CPUState* env, gva_t task){
  gpid_t pid;
  gpid_t parentPid;
  gpid_t tgid;
  gpid_t glpid;
  ...
  //update the info if needed
  if ((findProcessByPID(pid) == NULL)) // i.e. it doesn't exist
  {
    addProcess(i, pid, parentPid, tgid, glpid, uid, gid, euid, egid, pgd, (argName[0] == '\0') ? NULL : argName, (name[0] == '\0') ? NULL : name);
    updateMask |= UPDATE_THREADS | UPDATE_MODULES;
    //the wanted process starts running
		start_tracing_pid(pid);
  }
  else
  {
...
void updateProcessList(CPUState* env, gpa_t newpgd, int updateMask)
{
  ...
  target_ulong uid;
	...
	/* ARTDS START */
  task = DECAF_get_next_task_struct(env,task);
	/* ARTDS END */
  gva_t i = task;
...
/* ARTDS START */
void artdsSwitchProcess(CPUState* env, gpa_t newpgd){
  if ((env == NULL) || (env->regs[15] < 0xC0000000))
  {
    return;
  }

  gva_t task = DECAF_get_current_process(env);
	// DECAF_printf("DBG: Get new task = 0x%08x\n", task);
  if (task == 0)
  {
    return;
  }
  ...
  do
  {
    ...
    if (pgd == pgd_strip(newpgd))
    {
      ...
      curProcessPGD = pgd;
      curProcessUID = uid;
      //if the new process is the one being tracing
      if (uid == uid_wanted) { // modified -- zhouhao
          artdsUpdateTracedProcessByTask(env, i);
		  }
    }
    i = DECAF_get_next_task_struct(env, i);	
  } while ( (i != task) && ( i != 0) );
...
void Context_PGDWriteCallback(DECAF_Callback_Params* params)
{
  ...
  artdsSwitchProcess(params->pgd.env, params->pgd.newPGD);
...
void context_init()
{
  ...
  initDump();
  /* ARTDS END */
  DECAF_set_output_file("record.log");
